...
所以我搞了这套 vibe‑coding 九阳神功——教你怎么管住 AI，让它老老实实交付。
...
vibe-coding 里，"抄黑话"就是学会把这些词当按钮：你一说"前后端分离 / 数据库迁移 / 任务队列"，AI 就应该自动补齐对应的目录、配置、脚本、验收步骤——最后能 `docker compose up` 跑起来、能 `curl` 验收。背概念没用，能触发 AI 干活的词才有用。
